...
from utils.output_vectors import (positive_answers, negative_answers,
                                neutral_answers)
...
def get_word_polarity(word_input):
    ...
    data = {}
    with open('corpora/SentiLex-lem-PT02.txt', 'r') as f:
...
def get_the_right_answer(proposition):
    """
    Baseado em uma determinada entrada de texto, calcula a positividade ou
    negatividade do texto montando uma resposta coerente com o contexto.
    """
    text_emotion = text_classifier(proposition)

    if text_emotion < 0:
        answer = '{}{}'.format(
            choice(negative_answers[0]),
            choice(negative_answers[1])
        )
    elif text_emotion > 0:
        answer = '{}{}'.format(
            choice(positive_answers[0]),
            choice(positive_answers[1])
        )
    else:
        answer = '{}{}{}'.format(
            choice(neutral_answers[0]),
            choice(neutral_answers[1]),
            choice(neutral_answers[2]),
        )
    return answer
...
def binary_wordmatch(input_text, word_list):
    ...
    for word in input_text:
        if any(re.fullmatch(word, target) for target in word_list):
            match = True
            break
        elif any(re.fullmatch(f'{prev_word} {word}', target) for target in word_list):
            match = True
            break
        prev_word = word
    return match
